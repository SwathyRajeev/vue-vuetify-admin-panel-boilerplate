...
export default {
  data: () => ({
    ...
    nameRules: [
      ...
      (v) => (v && v.length <= 20) || "State must be less than 20 characters",
    ],
  }),
  ...
  methods: {
    ...
    deleteItemConfirm() {
      axios
        .delete("states/" + this.editedItem.id)
        .then((res) => {
          if (res.code === 200) {
            // this.$swal(res.data.message);
            this.alert.color = "green--text";
            this.alert.title = "Success";
            this.alert.message = "State Deleted";
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        await axios
          .patch("states/" + this.editedItem.id, {
        ...
        if (
          ...
          this.alert.color = "red--text";
          this.alert.title = "Error";
          this.alert.message = "Please fill State";
          this.alert.type = true;
        } else {
          let data = {
            label: this.editedItem.label,
          ...
          await axios
            ...
            .then((res) => {
              if (res.data) {
                ...
                this.alert.color = "green--text";
                this.alert.title = "Success";
                this.alert.message = "State " + res.statusText;
                this.alert.type = true;
                console.log("res");
                console.log(res);
